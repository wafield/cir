...
def enter_forum(request, forum_url, phase_name):  # access /forum_name
  ...
  try:
    ...
    context[
      'error_msg'] = 'The requested forum is not valid: <b>' + forum_url + '</b>'
    return render(request, 'error_index.html', context)
  if forum.access_level == 'private' and (
      not request.user.is_authenticated() or not Role.objects.filter(
    user=request.user,
    forum=forum).exists()):
    context['load_error'] = '403'
    return render(request, 'error_index.html', context)
  if phase_name is not None and phase_name not in ['free_discuss', 'nugget',
                                                   'extract', 'categorize',
                                                   'improve', 'finished']:
    context['load_error'] = '404'
    context[
      'error_msg'] = 'The requested forum phase is not valid: <b>' + phase_name + '</b>'
    return render(request, 'error_index.html', context)
  ...
  for phase_name_tmp in ['free_discuss', 'nugget', 'extract', 'categorize',
                         'improve', 'finished']:
    if (ComplexPhase.objects.filter(forum=forum,
                                    name=phase_name_tmp).count() == 0):
      complexPhase = ComplexPhase(name=phase_name_tmp, description="",
                                  start_time=timezone.now(),
                                  end_time=timezone.now(), forum=forum)
      complexPhase.save()
...
def _get_phases(forum, selected_phase):
  ...
  results['phases'] = []
  for name in ['nugget', 'extract', 'improve']:
    ...
    if name == selected_phase:
      phase_info['status'] = 'active'
        ...
        (name == 'improve' and (
            forum.phase == 'improve' or forum.phase == 'finished')) or \
...
def enter_workbench(request, forum_url):  # access /forum_name
  ...
  if request.user.is_authenticated():
    ...
    for staff in forum.members.filter(
        Q(role='facilitator') | Q(role='admin')).exclude(user=request.user):
    ...
    try:
      ...
      UserInfo.objects.create(user=request.user, last_visited_forum=forum)
    try:
      request.session['role'] = Role.objects.get(user=request.user,
                                                 forum=forum).role
    except:
      pass
    context['user_id'] = request.user.id
...
def enter_statement(request, forum_url):
  ...
  findings = Claim.objects.filter(forum=forum, stmt_order__isnull=False,
                                  claim_category='finding').order_by(
    'stmt_order')
  for claim in findings:
    context['claims']['findings'].append(claim.getAttrStmt())
  pros = Claim.objects.filter(forum=forum, stmt_order__isnull=False,
                              claim_category='pro').order_by('stmt_order')
  for claim in pros:
    context['claims']['pros'].append(claim.getAttrStmt())
  cons = Claim.objects.filter(forum=forum, stmt_order__isnull=False,
                              claim_category='con').order_by('stmt_order')
  for claim in cons:
